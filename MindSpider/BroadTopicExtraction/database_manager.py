...
class DatabaseManager:
    ...
    def save_daily_topics(self, keywords: List[str], summary: str, extract_date: date = None) -> bool:
        ...
        try:
            ...
            # 生成话题ID: 日期_综合
            topic_id = f"{extract_date.isoformat()}_综合话题"
            topic_name = f"{extract_date.isoformat()}日热点话题"
            ...
            if existing:
                ...
                insert_query = """
                    INSERT INTO daily_topics (
                        topic_id, topic_name, extract_date, keywords, summary,
                        add_ts, last_modify_ts, news_count
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """
                cursor.execute(insert_query, (
                    topic_id, topic_name, extract_date, keywords_json, summary,
                    current_timestamp, current_timestamp, len(keywords)
                ))
